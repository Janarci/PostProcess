...
void Minigame::onCreate()
{
	...
	*/
	...
	VertexMesh quad_vertex_list[] = {
		VertexMesh(Vector3D(-1,-.5,0), Vector2D(0,1), Vector3D(),Vector3D(),Vector3D()),
		VertexMesh(Vector3D(-1,.5,0), Vector2D(0,0), Vector3D(),Vector3D(),Vector3D()),
		VertexMesh(Vector3D(1,.5,0), Vector2D(1,0), Vector3D(),Vector3D(),Vector3D()),
		VertexMesh(Vector3D(1,-.5,0), Vector2D(0,1), Vector3D(),Vector3D(),Vector3D())
	};
	...
	m_render_target = GraphicsEngine::get()->getTextureManager()->createTexture(Rect(width, height), Texture::Type::RenderTarget);
	m_depth_stencil = GraphicsEngine::get()->getTextureManager()->createTexture(Rect(width, height), Texture::Type::DepthStencil);

	m_post_process_mat->addTexture(m_render_target);

}

void Minigame::onUpdate()//draw mesh
{
	Window::onUpdate();
